/**
 * PaxDesign Utility Dock — v2.0.0
 * Initializes from PDX_CONFIG injected by wp_localize_script.
 */
(function () {
  'use strict';
  ...
  if (!root || !backdrop || !panel || !inner) return;
    ...
    return (
      '<div class="pdx-ph">' +
        '<div class="pdx-ph__left">' +
          ...
          '</div>' +
        '</div>' +
        '<button class="pdx-ph__close" id="pdx-close" type="button" aria-label="Close panel">' + ICONS.x + '</button>' +
      '</div>'
    );
  }
  ...
  function buildServicePanel(moduleId) {
    ...
    if (!svc) return '<div class="pdx-error">Module not found.</div>';

    var mod  = CFG.modules[moduleId] || {};
    var icon = mod.icon || 'circle';

    var featureItems = svc.features.map(function (f) {
      return '<li>' + esc(f) + '</li>';
    }).join('');
  ...
  function runScan() {
    ...
    // Use WP REST proxy to avoid CORS issues
    ...
    .then(function (data) {
      ...
      outputEl.innerHTML = buildTrustResult(parsed.host, parsed.href, data.rdap, data.ssl);
    })
    .catch(function () {
  ...
  function buildTrustResult(domain, href, rdap, ssl, gsb) {
    ...
    /* Tags */
    var tags =
      good.map(function (t) { return '<span class="pdx-tag pdx-tag--green">'  + esc(t) + '</span>'; }).join('') +
      warn.map(function (t) { return '<span class="pdx-tag pdx-tag--yellow">' + esc(t) + '</span>'; }).join('') +
      bad.map(function  (t) { return '<span class="pdx-tag pdx-tag--red">'    + esc(t) + '</span>'; }).join('');
    ...
    var rowsHTML = rows.map(function (r) {
      return (
        '<div class="pdx-result__row">' +
          '<span class="pdx-result__row-key">' + esc(r.k) + '</span>' +
          '<span class="pdx-result__row-val' + (r.c ? ' pdx-result__row-val--' + r.c : '') + '">' + esc(r.v) + '</span>' +
        '</div>'
      );
    }).join('');
    ...
    return (
      '<div class="pdx-result">' +
        ...
        '</div>' +
        '<div class="pdx-result__bar">' +
          '<div class="pdx-result__fill pdx-result__fill--' + cls + '" style="width:' + score + '%"></div>' +
        '</div>' +
  ...
  document.addEventListener('click', function (e) {
    ...
    if (btn && root.contains(btn)) {
      openPanel(btn.getAttribute('data-module'));
      return;
    }

    /* Close button */
    if (e.target.closest('#pdx-close')) {
      closePanel();
      return;
    }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === 'Enter' && e.target.id === 'pdx-trust-input') { runScan(); }
  });

  /* Mobile: prevent body scroll bleed */
